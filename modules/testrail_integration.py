...
def execute_changes(testrail_session: TestRail, changelist):
    ...
    if len(config_matches) == 1:
        ...
        raise ValueError(f"Should only have one matching TR config: {changelist.get('config')}")
            ...
            for entry in suite_entries:
                ...
                if not entry.get("runs"):
                    ...
                    if entry_changes[suite_id].get("cases_to_add_to_run"):
                        entry_changes[suite_id]["cases_to_add_to_run"].append(int(test_case))
                    else:
                        entry_changes[suite_id]["cases_to_add_to_run"] = [int(test_case)]
                    continue
